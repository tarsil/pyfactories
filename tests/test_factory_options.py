...
def test_factory_handling_of_optionals() -> None:
    ...
    assert any(
        r.complex[0] is not None for r in [FactoryWithNoneOptionals.build() for _ in range(10)]
    )
    ...
    assert all(
        r.name is not None for r in [FactoryWithoutNoneOptionals.build() for _ in range(10)]
    )
    assert all(r.id is not None for r in [FactoryWithoutNoneOptionals.build() for _ in range(10)])
    assert any(
        r.complex[0] is not None for r in [FactoryWithoutNoneOptionals.build() for _ in range(10)]
    )
