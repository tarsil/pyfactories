...
def test_is_union() -> None:
    ...
    for field_name, model_field in UnionTestFactory.get_model_fields(
        UnionTestFactory._get_model()
    ):
        if field_name == "union":
    ...
    if sys.version_info >= (3, 10):
        ...
        for field_name, model_field in UnionTestWithPipeFactory.get_model_fields(
            UnionTestWithPipeFactory._get_model()
        ):
...
def test_is_multiply_of_multiple_of_in_range_extreme_cases() -> None:
    ...
    assert not is_multiply_of_multiple_of_in_range(
        minimum=Decimal(1), maximum=Decimal(1), multiple_of=Decimal("0.333")
    )
...
def test_is_multiply_of_multiple_of_in_range_for_floats(
    base_multiple_of: float, multiplier: int
) -> None:
    if multiplier != 0:
...
def test_is_multiply_of_multiple_of_in_range_for_int(
    base_multiple_of: int, multiplier: int
) -> None:
...
def test_is_multiply_of_multiple_of_in_range_for_decimals(
    base_multiple_of: Decimal, multiplier: int
) -> None:
        ...
        for multiple_of in [base_multiple_of, -base_multiple_of]:
            minimum, maximum = sorted(
                [
                    ...
                    (multiplier + random.randint(1, 100)) * multiple_of
                    + Decimal(random.random() * 100),
                ]
            )
            assert is_multiply_of_multiple_of_in_range(
                minimum=minimum, maximum=maximum, multiple_of=multiple_of
