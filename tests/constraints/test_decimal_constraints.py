...
def test_handle_constrained_decimal_handles_max_digits(max_digits: int) -> None:
    if max_digits > 0:
        ...
        assert (
            len(result.as_tuple().digits) - abs(cast("int", result.as_tuple().exponent))
            <= max_digits
        )
    else:
        with pytest.raises(ParameterError):
...
def test_handle_constrained_decimal_handles_max_digits_and_decimal_places(
...
def test_handle_constrained_decimal_handles_multiple_of_with_lt(
...
def test_handle_constrained_decimal_handles_multiple_of_with_le(
    val1: Decimal, val2: Decimal
) -> None:
    multiple_of, max_value = sorted([val1, val2])
    if multiple_of != Decimal("0"):
...
def test_handle_constrained_decimal_handles_multiple_of_with_ge(
    val1: Decimal, val2: Decimal
) -> None:
...
def test_handle_constrained_decimal_handles_multiple_of_with_gt(
    val1: Decimal, val2: Decimal
) -> None:
    min_value, multiple_of = sorted([val1, val2])
    if multiple_of != Decimal("0"):
